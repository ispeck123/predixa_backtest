...
---
## 0. HARD NUMBERS (do not change without Apratim sign-off)
...
---
## 1B. FUTURES = OPTION 3 (single-lot mechanism check) — OVERRIDES the §3 throttle for now
Rs50k ceiling cannot fund futures VOLUME (margin ~Rs80-95k PER LOT exceeds the whole ceiling).
So futures is NOT a statistical track yet — it is a ONE-LOT MECHANISM CHECK: prove the futures
execution path works live (GTT Single on a futures contract -> WS fill -> GTT OCO -> exit).
Contributes NOTHING to any tracking-gate (n=1 is not a sample). Cash is the statistical track.

RULES (strict serial — zero concurrency by construction):
  1. Symbol pool: the LOWEST per-lot-risk validated names (per futures_median_stop_pct.csv x live
     lot/price). Cheapest by margin: ITC (~Rs81k) / HDFC (~Rs82k). Pick the lowest that passes
     liquidity floor. Long only.
  2. Place EXACTLY ONE futures GTT Single entry at a time (top-ranked signal). NEVER a second.
     -> double-fill is IMPOSSIBLE because a second GTT is never placed. This is the safety
        guarantee (not a cancel-after-fill race).
  3. If it FILLS  -> futures LOCKED -> run to completion (SL or target) -> only THEN eligible to
     place the next single GTT.
  4. If it does NOT fill within the staleness window (match cash, ~2-3 trading days) -> CANCEL it
     -> place the next-ranked candidate's single GTT.
  5. Never more than ONE futures GTT resting OR filled at any moment.

CAPITAL REALITY (state to Apratim, do not obscure):
  - Margin BLOCKED to hold 1 lot: ~Rs81-95k (exceeds the Rs50k LOSS ceiling — this is margin
    tied up, NOT money at risk).
  - Money AT RISK on the trade: ~Rs6-8k (one stop). Well under Rs50k.
  - So: ~Rs81k tied up for the ~2-day life of the trade; ~Rs8k at risk. Confirm comfort before start.

The Rs50k HARD CEILING (cash+futures REALIZED loss -> halt all) still applies unconditionally.
The §3 dynamic throttle is PARKED — it activates only if/when Option 2 (multi-lot statistical
futures, a deliberate ~Rs1.6-2.9L standing-margin decision) is later chosen. Not now.
...
---
## 3. FUTURES DYNAMIC THROTTLE  [PARKED — activates only under Option 2, not the current Option-3 run]
(state machine — evaluate BEFORE every futures fire)
