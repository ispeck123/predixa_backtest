...
# ----------------------------------------------------------------------
# CSV replay loader for MFE / MAE / bars-after-fill
# Expected files:
#   <csv_data_dir>/<SYMBOL>_<TIMEFRAME>.csv
# Example:
#   RELIANCE_fifteen.csv
#   TCS_sixty.csv
#   INFY_seventy_five.csv
#
# CSV should contain:
#   datetime/time/date column
#   high
#   low
#   (optional close/open)
# ----------------------------------------------------------------------
...
_FYERS_EXECUTION_CACHE = {}

def load_fyers_execution_log(path: Optional[str]):
    """Load broker fills from Fyers order response log."""
    global _FYERS_EXECUTION_CACHE
    if not path:
        logger.warning("Fyers log disabled")
        return
    ...
    with open(path, "r", errors="ignore") as fh:
        for line in fh:
            ...
            if not isinstance(obj, dict):
                continue
            payload = obj.get("orders") or obj.get("trades")
            if not isinstance(payload, dict):
                continue
            oid = str(payload.get("id") or payload.get("orderNumber") or "")
            if not oid:
                continue
            _FYERS_EXECUTION_CACHE[oid] = {
                "broker_order_id": oid,
                "exchange_order_id": payload.get("exchOrdId") or payload.get("exchangeOrderNo"),
                "broker_symbol": payload.get("symbol"),
                "order_tag": payload.get("orderTag"),
                "broker_status": payload.get("status"),
                "filled_qty": payload.get("filledQty") or payload.get("tradedQty"),
                "actual_fill_price": payload.get("tradedPrice") or payload.get("tradePrice"),
                "broker_fill_timestamp": payload.get("orderDateTime"),
            }
            count += 1
    logger.info("Fyers execution cache loaded=%s", len(_FYERS_EXECUTION_CACHE))

def get_fyers_execution(order):
    return _FYERS_EXECUTION_CACHE.get(str(order.order_id), {})


logger = logging.getLogger("mfe_mae_replay")
...
def normalize_timeframe_name(value: Any) -> Optional[str]:
    text = str(value).strip().lower().replace("-", "_").replace(" ", "_")
    aliases = {
        "1": "daily", "daily": "daily",
        "2": "sixty", "60": "sixty", "sixty": "sixty",
        "3": "fifteen", "15": "fifteen", "fifteen": "fifteen",
        "5": "one_twenty_five", "125": "one_twenty_five", "one_twenty_five": "one_twenty_five",
        "6": "twenty_five", "25": "twenty_five", "twenty_five": "twenty_five",
        "75": "seventy_five", "seventy_five": "seventy_five", "seventyfive": "seventy_five",
    }
    return aliases.get(text)


# IMPORTANT:
# Timeframe names can contain multiple underscore parts.
# Example:
# pnb_housing_finance_one_twenty_five.csv
# Therefore do not use rsplit('_',1).
TIMEFRAME_SUFFIXES = sorted(
    [
        "one_twenty_five",
        "seventy_five",
        "twenty_five",
...
def extract_symbol_timeframe(filename: str):
    ...
    for tf in TIMEFRAME_SUFFIXES:
        ...
        if stem.endswith(suffix):
            symbol = stem[:-len(suffix)].strip("_")
            return symbol, tf
...
def csv_key(symbol: str, timeframe: int) -> str:
    return f"{normalize_symbol(symbol)}_{TIMEFRAME_FILE_MAP.get(int(timeframe), str(timeframe))}"


def load_csv_data_directory(csv_data_dir: Optional[str]) -> None:
    if not csv_data_dir:
        logger.warning("CSV replay disabled. --csv-data-dir not provided")
        return

    root = Path(csv_data_dir)
    if not root.exists():
        raise FileNotFoundError(f"CSV data directory not found: {root}")

    files = list(root.glob("*.csv"))
    _PRICE_CACHE_STATS["files_scanned"] = len(files)
    logger.info("CSV replay directory=%s files=%s", root, len(files))

    for file in files:
        try:
            symbol, tf_name = extract_symbol_timeframe(file.name)

            if not symbol:
                logger.warning("Unknown timeframe in filename: %s", file.name)
                continue

            tf = normalize_timeframe_name(tf_name)
            if tf is None:
                logger.warning("Unknown timeframe in filename: %s", file.name)
                continue

            logger.debug("Loading CSV %s", file)

            df = pd.read_csv(file)

            time_col = next(
                (c for c in df.columns if c.lower() in {
                    "datetime", "date", "time", "timestamp", "tradedate"
                }),
                None,
            )
            ...
            if not time_col or not high_col or not low_col:
                logger.error(
                    "Invalid CSV %s missing columns. Found=%s",
                    file.name,
                    list(df.columns),
                )
                _PRICE_CACHE_STATS["files_failed"] += 1
                continue
            ...
            df = (
                df.dropna(subset=["_dt", high_col, low_col])
                .sort_values("_dt")
                .reset_index(drop=True)
            )

            key = f"{normalize_symbol(symbol)}_{tf}"
            ...
            _PRICE_CACHE_STATS["files_loaded"] += 1
            _PRICE_CACHE_STATS["rows_loaded"] += len(df)

            logger.info(
                "Loaded %s => key=%s candles=%s range=%s to %s",
                file.name,
                key,
                len(df),
                df["_dt"].min(),
            ...
            _PRICE_CACHE_STATS["files_failed"] += 1
            logger.exception("Failed loading CSV %s", file)

    logger.info("CSV CACHE READY %s", _PRICE_CACHE_STATS)

def calculate_replay_metrics(symbol, timeframe, side, entry_price, stoploss_price, fill_time, exit_time):
    key = csv_key(symbol, timeframe)
    source = _PRICE_CACHE.get(key)

    fill_dt = parse_datetime_value(fill_time) or parse_datetime_value(exit_time)

    if not source:
        logger.warning("No candle data found for key=%s symbol=%s tf=%s", key, symbol, timeframe)
        return {"mfe_R": None, "mae_R": None, "bars_after_fill": None}

    if fill_dt is None:
        logger.warning("No fill timestamp for order symbol=%s", symbol)
        return {"mfe_R": None, "mae_R": None, "bars_after_fill": None}

    df = source["df"]
    future = df[df["_dt"] >= fill_dt]

    exit_dt = parse_datetime_value(exit_time)
    if exit_dt:
        future = future[future["_dt"] <= exit_dt]

    logger.info(
        "Replay %s | side=%s entry=%s fill=%s bars=%s",
        key, side, entry_price, fill_dt, len(future)
    )

    if future.empty:
        return {"mfe_R": None, "mae_R": None, "bars_after_fill": 0}

    high = future[source["high"]].astype(float)
    low = future[source["low"]].astype(float)

    if side == "BUY":
        mfe = high.max() - entry_price
        mae = low.min() - entry_price
    else:
        mfe = entry_price - low.min()
        mae = entry_price - high.max()

    risk = abs(entry_price - stoploss_price) if stoploss_price else None

    result = {
        "mfe_R": round(mfe / risk, 4) if risk else None,
        "mae_R": round(mae / risk, 4) if risk else None,
        "bars_after_fill": int(len(future)),
    }

    logger.info("Replay result %s => %s", key, result)
    return result
...
def completed_execution_row(order: OrderRow, engine_record: Mapping[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    fyers = get_fyers_execution(order)
    actual_fill = fyers.get("actual_fill_price") or filled_entry
    if actual_fill:
        filled_entry = safe_float(actual_fill)
    ...
    return {
        ...
        "broker_order_id": fyers.get("broker_order_id"),
        "exchange_order_id": fyers.get("exchange_order_id"),
        "broker_symbol": fyers.get("broker_symbol"),
        "broker_order_tag": fyers.get("order_tag"),
        "broker_status": fyers.get("broker_status"),
        "broker_filled_qty": fyers.get("filled_qty"),
        "broker_fill_timestamp": fyers.get("broker_fill_timestamp"),
        "fill_timestamp": fyers.get("broker_fill_timestamp") or datetime_text(order.entry_timestamp),
        ...
        **{
            "mfe_R": status.get("mfe_R") if status.get("mfe_R") is not None else calculate_replay_metrics(
                order.stock_tick,
                order.time_frame,
                order.order_type,
                filled_entry,
            ...
            "mae_R": status.get("mae_R") if status.get("mae_R") is not None else calculate_replay_metrics(
                order.stock_tick,
                order.time_frame,
                order.order_type,
                filled_entry,
            ...
            "bars_after_fill": status.get("bars_after_fill") if status.get("bars_after_fill") is not None else calculate_replay_metrics(
                order.stock_tick,
                order.time_frame,
                order.order_type,
                filled_entry,
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--csv-data-dir", default=None, help="Directory containing SYMBOL_TIMEFRAME.csv OHLC files for MFE/MAE replay")
    parser.add_argument("--fyers-log", default=None, help="Fyers order response log for real broker fills")
...
def main() -> None:
    ...
    load_csv_data_directory(args.csv_data_dir)
    load_fyers_execution_log(args.fyers_log)
    logger.info("Loaded candle cache keys sample=%s", list(_PRICE_CACHE.keys())[:20])
