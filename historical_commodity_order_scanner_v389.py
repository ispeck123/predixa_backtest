"""
Parallel historical Commodity Contract scanner -> existing commodity_order_master.

Updated commodity version:
...
- Database writes use ONLY the parent process through
  insert_future_and_commodity_order(..., segment="commodity").
- Worker always calls process_setup_fc(..., is_future=False).
...
- Supports single or multiple commodity timeframes.

Run from the PROJECT ROOT on Ubuntu:
    python backtest/historical_commodity_order_scanner_v389.py --start-date 2026-06-01 --end-date 2026-07-06 --side BOTH
"""
...
FIXED_COMMODITY_EXPIRIES: Dict[str, List[str]] = {
    "GOLDM": ["2026-08-05", "2026-09-04", "2026-10-05"],
    "GOLDTEN": ["2026-07-31", "2026-08-31", "2026-09-30"],
    "SILVER": ["2026-07-31", "2026-08-31", "2026-09-04"],
    "SILVERM": ["2026-08-31", "2026-11-30", "2027-02-26"],
    "CRUDEOILM": ["2026-07-20", "2026-08-19", "2026-09-21"],
    "NATGASMINI": ["2026-07-28", "2026-08-26", "2026-09-25"],
    "ZINCMINI": ["2026-07-31", "2026-08-31", "2026-09-30"],
}

COMMODITY_STACKS: Dict[str, Dict[str, Any]] = {
    "M-W-D": {"time_frame": 1, "time_list": ["monthly", "weekly", "daily"]},
    "W-D-240": {"time_frame": 2, "time_list": ["weekly", "daily", "two_forty"]},
    "D-240-120": {"time_frame": 3, "time_list": ["daily", "two_forty", "one_twenty"]},
    "D-240-60": {"time_frame": 4, "time_list": ["daily", "two_forty", "sixty"]},
}

COMMODITY_TIME_FRAME_IDS: Dict[str, int] = {
    "daily": 1,
    "two_forty": 2,
    "one_twenty": 3,
    "sixty": 4,
...
@dataclass
class ParallelCommodityScanJob:
    start_date: date
    end_date: date
    stack_items: List[Tuple[str, int, List[str]]]
    country_id: int = 1
    scan_time: clock_time = clock_time(23, 20)
    ...
    output_path: str = "outputs/commodity_backtest_order_scanner_v389.jsonl"
    workers: int = 4
    max_in_flight: int = 16
    progress_every: int = 25
    start_method: str = "spawn"
    fixed_expiry_map: Dict[str, List[str]] = field(default_factory=lambda: copy.deepcopy(FIXED_COMMODITY_EXPIRIES))
...
def parse_clock_time(value: str) -> clock_time:
    try:
        ...
        raise argparse.ArgumentTypeError("scan time must be HH:MM, for example 23:20") from exc
...
def selected_stacks(time_frame: Optional[str], stack_code: Optional[str]) -> List[Tuple[str, int, List[str]]]:
    ...
    if stack_code:
        ...
        for code in requested_codes:
            if code not in COMMODITY_STACKS:
                raise ValueError(f"Unknown stack code: {code}. Valid: {', '.join(COMMODITY_STACKS.keys())}")
            meta = COMMODITY_STACKS[code]
            items.append((code, int(meta["time_frame"]), list(meta["time_list"])))
        return items
    if time_frame:
        requested_tfs = {int(x.strip()) for x in time_frame.split(",") if x.strip()}
        for code, meta in COMMODITY_STACKS.items():
            if int(meta["time_frame"]) in requested_tfs:
                items.append((code, int(meta["time_frame"]), list(meta["time_list"])))
        if not items:
            raise ValueError(f"No commodity stacks found for --time-frame {time_frame}. Valid: 1,2,3,4")
        return items
    return [(code, int(meta["time_frame"]), list(meta["time_list"])) for code, meta in COMMODITY_STACKS.items()]
...
def validate_stack_item(item: Tuple[str, int, List[str]]) -> Tuple[str, int, List[str]]:
    ...
    if execute_tf not in COMMODITY_TIME_FRAME_IDS:
        raise ValueError(f"Unsupported commodity execute timeframe {execute_tf!r}. Allowed: {', '.join(COMMODITY_TIME_FRAME_IDS)}")
    expected_time_frame = COMMODITY_TIME_FRAME_IDS[execute_tf]
...
def get_trade_dict(formatted: Mapping[str, Any], side: str) -> Dict[str, float]:
    ...
    if missing:
        raise ValueError(f"{side} setup is missing: {', '.join(missing)}. Available keys: {sorted(flatten_mapping(formatted).keys())}")
...
def side_allowed(side: str, side_mode: str, allow_sell: bool) -> bool:
    ...
    if side_mode == "BOTH":
        return side == "BUY" or (side == "SELL" and allow_sell)
    return False
...
def process_scan_task(task: ScanTask) -> Dict[str, Any]:
    ...
    record: Dict[str, Any] = {
        ...
        "data_source": "process_setup_fc(is_future=False): commodity data branch",
    }
    try:
        raw = process_setup_fc(task.symbol, list(task.time_list), task.exp_num, task.scan_at, False)
        ...
        for side, rrr in qualified_sides(formatted, task.min_rrr, task.side_mode, task.allow_sell):
...
def build_task_iterator(job: ParallelCommodityScanJob) -> Iterator[ScanTask]:
    symbol_filter = normalise_symbol(job.symbol_filter) if job.symbol_filter else None
    segment = resolve_segment(is_future=False)
    allow_sell = SIDE_POLICY.is_enabled(segment, Side.SHORT) if job.respect_sell_policy else True
...
def count_skipped_near_expiry(job: ParallelCommodityScanJob) -> int:
    symbol_filter = normalise_symbol(job.symbol_filter) if job.symbol_filter else None
    skipped = 0
    for scan_day in iter_days(job.start_date, job.end_date, job.step_days, job.weekdays_only):
        for symbol, expiry_texts in job.fixed_expiry_map.items():
...
def handle_result(result: Dict[str, Any], job: ParallelCommodityScanJob, audit: Any, stats: Dict[str, Any], dry_run: bool) -> None:
    ...
    for setup in setups:
        ...
        try:
            ...
            logger.error(
                "Commodity backtest order insert failed | %s | %s | %s | %s",
                result.get("symbol"),
                result.get("exp_num"),
                result.get("scan_at"),
            ...
            insert_record.update({"insert_status": "INSERT_ERROR", "insert_error": str(exc)})
...
def run(job: ParallelCommodityScanJob, dry_run: bool = False) -> Dict[str, Any]:
    ...
    stats: Dict[str, Any] = {
        "target_table": "commodity_order_master",
        "segment": "commodity",
        ...
        "min_rrr": job.min_rrr,
        "cash_gate_applied": False,
        "time_lists": [{"stack_code": code, "time_frame": time_frame, "time_list": time_list} for code, time_frame, time_list in job.stack_items],
        "time_frame_mapping": COMMODITY_TIME_FRAME_IDS,
        "data_path_mode": "process_setup_fc(is_future=False): commodity data branch",
    ...
    task_iterator = build_task_iterator(job)
            ...
            while pending:
                ...
                for future in done:
                    ...
                    try:
                        ...
                        logger.error("Commodity worker crashed for %s: %s", task, exc, exc_info=True)
                        result = {
                            "status": "ERROR",
                            "scan_at": task.scan_at,
                            "symbol": task.symbol,
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Parallel historical commodity scanner -> commodity_order_master. BUY and SELL both enabled by default.")
    ...
    parser.add_argument("--country-id", type=int, default=1, help="country_id stored in commodity_order_master")
    parser.add_argument("--scan-time", type=parse_clock_time, default=clock_time(23, 20), help="Historical candle cut-off, HH:MM; default 23:20")
    parser.add_argument("--time-frame", default=None, help="Optional selected DB timeframes. Example: 1 or 1,2,3,4. Default all.")
    parser.add_argument("--stack-code", default=None, help="Optional selected stack codes. Example: M-W-D or W-D-240,D-240-60. Default all.")
    ...
    parser.add_argument("--respect-sell-policy", action="store_true", help="If passed, SELL follows SideEnablementPolicy. Default false, so commodity SELL is enabled.")
    parser.add_argument("--symbol", default=None, help="Optional test filter, example: GOLDM")
    ...
    parser.add_argument("--output-path", default="outputs/commodity_backtest_order_scanner_v389.jsonl")
...
def main() -> None:
    ...
    try:
        ...
        print("\nPARALLEL HISTORICAL COMMODITY ORDER SCAN COMPLETED")
        print(json.dumps(result, default=json_default, indent=2))
    except Exception as exc:
        logger.exception("Parallel historical commodity order scan stopped: %s", exc)
        raise SystemExit(f"PARALLEL HISTORICAL COMMODITY ORDER SCAN FAILED: {exc}")
